...
class MCTS:
    ...
    def initialize_tree(
        ...
        self.update_tree_node(tree, Tree.ROOT_INDEX, prior_logits, hidden_state, value)
        return tree
    ...
    def expand(
        ...
        tree = self.update_tree_node(
            tree, next_node_index, prior_logits, hidden_state, value
        )
        return replace(
            tree,
    ...
    def backward(self, tree: Tree, leaf_index: int) -> Tree:
    ...
    def search(
        ...
        tree = self.initialize_tree(
            ...
            hidden_state, value, legal_actions, gumbel_noise
        )
        for sim in range(self.config.simulations):
            node_index, action = self.simulate(tree, action_selection_func)
            next_node_index = tree.children_index[node_index, action]
            ...
            tree = self.expand(
                ...
                hidden_state, reward, value
            )
            tree = self.backward(tree, next_node_index)
