...
class Node:
    ...
    def expand(self,
               reward: float,
               hidden_state: torch.Tensor,      # (1 x nodes x embedding_size)
               policy_logits: torch.Tensor,     # (action_space_size)
...
class MCTS:
    ...
    def select_action(self, root: Node, temperature: float) -> ActType:
        ...
        if temperature == 0:
            ...
            idx = np.random.choice(
                range(len(visit_counts.keys())), p=[v / total_visits for v in visit_counts.values()]
            )
    ...
    def search(self,
        ...
        # (1 x action_space_size), (1 x nodes x embedding_size), (1)
        policy_logits, hidden_state, value = network.initial_inference(
            Batch.from_data_list([observation])
        )

        root.expand(0, hidden_state, policy_logits.squeeze(0), legal_actions)
        ...
        for _ in range(self.config.simulations):
            ...
            policy_logits, hidden_state, value, reward = network.recurrent_inference(
                parent.hidden_state,
                ftensor(action_encoder(history[-1])).unsqueeze(0).unsqueeze(-1)
            )
            node.expand(reward, hidden_state, policy_logits.squeeze(0), range(self.config.action_space_size))
